...
## Job
1. **Read the lessons.** For each in-scope agent, read its `MEMORY.md`. Each lesson carries an address tag (`target:` + `layer:`) per the `gspec-memory` convention. Ignore untagged noise. Also read `.gspec/agent-runs/feedback-log.md` if present — the subagent-capture hook's record of failing QA verdicts. Treat it as **corroborating evidence**, not a lesson itself: a failure mode that recurs there is strong signal a skill fix is warranted; use it to rank and justify proposals, not as a standalone source.
