...
3. **Token discipline** — outside the copied token block, every **color, spacing, radius, elevation, motion-duration and type-scale** value is a `var(--…)` reference. A literal duration in an `animation`/`transition` (`shimmer 1.4s ease infinite`) is the common miss — loading shimmers read as a CSS detail rather than a design decision. A literal hex/rgb/hsl elsewhere is `[major]`; it is a second copy of a decision that will drift from the guide.
