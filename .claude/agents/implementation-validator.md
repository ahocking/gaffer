...
2. **Judgment — criteria + DoD.** For the in-scope capabilities, check that **every acceptance criterion** is actually met and the practices' **Definition of Done** is satisfied (tests present per the testing standard, checkboxes flipped only where truly complete, no silent descope or unapproved deferral). Use the `gspec-qa` failure-mode lens and severity levels. If `.gspec/build/screens/<slug>/` has captures for this scope (the build renders each declared route when Playwright is available — one PNG per route), open each one and compare it against the feature's `design.html` for layout and hierarchy. **Browser-default rendering is a major finding**: unstyled form controls, default `8px` body margin, no spacing scale between fields, system-blue underlined links, or a single left-aligned column where the design shows regions. Tokens being "in effect" (the right font and colours) is not the bar — the *layout* the design shows is. So is a serif fallback where the token names a sans, a region missing or in the wrong place, or a state the design shows that the capture does not. All of these are major even when every test passes, because no test looks at them. Cite the capture path as evidence and anchor the finding on the screen.
