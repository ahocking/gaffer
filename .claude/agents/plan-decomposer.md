...
skills: [gspec-engineer, gspec-conventions, gspec-memory]
...
## Input
A feature slug, the exact path to write (`gspec/features/<slug>/tasks.md`), its PRD, and its architecture. If a plan is already there, read it first — every checked task is reproduced verbatim and new work is appended with the next free ID.
