...
1. **Inspect.** Delegate to the `codebase-inspector` agent with any scope hint from the arguments below. It reads the specs, inspects the code, and returns impact-ordered drift findings plus orphan capabilities (or reports that the specs match) — including **module** drift, where `architecture.md`'s Modules table (+ the committed `verify.sh`) no longer matches the real build/test toolchain, and **enrichment** drift, where a feature folder still names a technology the project has since moved off. If `gspec/` is empty, say so and stop.
