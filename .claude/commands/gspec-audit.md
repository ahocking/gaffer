...
1. **Inspect.** Delegate to the `codebase-inspector` agent with any scope hint from the arguments below. It reads the specs, inspects the code, and returns impact-ordered drift findings plus orphan capabilities (or reports that the specs match) — including **deployable** drift, where `architecture.md`'s Deployables table (+ the committed `verify.sh`) no longer matches the real build/test toolchain. If `gspec/` is empty, say so and stop.
