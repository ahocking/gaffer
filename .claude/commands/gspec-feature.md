...
2. **Assess scope** (the decomposition heuristic in `gspec-product`). Decide whether this is a single feature or a large body of work. If large, propose a breakdown (each feature + a one-line description + dependencies) and **ask the user to confirm / adjust / reprioritize before writing anything.** When in doubt, lean toward fewer features. (The autonomous build applies this same heuristic headlessly via the `feature-planner` agent.)
3. **Interview** (product judgment + the `gspec-authoring` clarification protocol — **one question per message**) to resolve users, scope/boundaries, capabilities, priorities, and success criteria — offer 2–3 suggestions per question. Resolve everything before writing; the PRD reflects decisions, not open questions.
