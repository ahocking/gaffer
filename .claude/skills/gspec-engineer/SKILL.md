...
- `T<a> and T<b> are both [P] and both write \`<file>\` — tasks marked to run alongside each other cannot write the same file`: name each task's files in backticks, and give two `[P]` tasks disjoint files or sequence them. The build drops the marker from the later task itself and says so.
