...
- **Fan-out (parallelism)** — two scopes may run **concurrently only when they are truly independent**: their dependencies are already complete AND they write **disjoint files** (no shared module, migration, or config). This is the plan-level echo of the engineer's `[P]` rule — *when in doubt, don't parallelize*; a false "independent" causes two runs to clobber each other, which costs far more than the lost concurrency. **Doubt is for the cases the evidence does not settle.** When the caller hands you a computed file-overlap table (the autonomous build does), a pair it lists as provably disjoint with no declared dependency is *not* in doubt — put them in the same wave. Serializing evidence-backed independent work is the opposite failure, and on a measured build it cost six waves for six disjoint features.
