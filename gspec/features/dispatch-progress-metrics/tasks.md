...
- [ ] **T18** **P1** Bring ADR 0019's v3.6 section up to what T13 and T14 shipped, editing only §4 and §5. In §4 (Null rules), state that `totals.dispatch_waste.over_threshold` (count and token share) reads null, with a `notes[]` reason naming the unresolved dispatch, whenever any counted implementer dispatch has a null `tool_calls` (T13), rather than excluding that dispatch. In §5 (The threshold stamp), state that `implementer_turn_budget` is read by one shared strict reader in `runstate.sh` (`_rs_read_turn_budget`, exposed as `runstate.sh turn-budget`), which `metrics.sh`'s `resolve_turn_threshold` calls rather than keeping its own copy and which `_rs_implementer_turn_budget` also calls; each caller keeps its own fallback, and a value counts only when, after a trailing comment, surrounding whitespace and one pair of surrounding quotes are removed, it is a positive integer (T14). Nothing else in the ADR changes
  - deps: —
  - covers: ADR 0019 v3.6 states what T13 and T14 shipped
  - arch: —
  - files: docs/adr/0019-run-metrics-observability.md
