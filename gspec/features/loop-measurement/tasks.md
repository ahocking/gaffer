...
- [ ] **T8** **P0** Run `sweep-open` in `run-loop` and `resume` immediately before every start or continuation record:
...
- [ ] **T9** [P] **P0** Add `scripts/spend.sh`, with a `scripts/test-spend.sh` sweep registered in CI, that does the following:
