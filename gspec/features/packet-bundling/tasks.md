...
- [x] **T8** [P] **P0** Make a bundle that did NOT land green report its later members as unmeasured rather than as measured zeros in `scripts/metrics.sh`: a non-green bundle has no commit and so no trailers, and its members reach the collector only through the record-only join (around the `recordjoin` block, lines ~703-765), where each gets `seq: 1` and so takes the measured path, with every member after the first reading real-looking zeros across one shared boundary. Group record-only ids by the `(session, ts)` of their latest terminal record, give each a `seq` ordinal from the outcomes-log line order (cursor first, which T3 writes in plan order), so members 2..n take the existing `$is_sibling` null path with the `unmeasured:shared-packet-boundary` flag, the same shape T1 gave trailer siblings, and never a second nulling shape; a record-only packet with no sibling at its boundary must stay byte-identical to today; add a `scripts/test-metrics.sh` case over a three-member rolled-back bundle (3 rows, rows 2-3 null not 0 and flagged, row 1 is the cursor and measured) plus a lone record-only packet asserting today's output unchanged.
  - deps: T1, T3
  - covers: Every task in a bundle keeps its own record · Run metrics emit one packet row per trailer on a commit
  - arch: —
  - files: scripts/metrics.sh, scripts/test-metrics.sh
- [x] **T9** [P] **P0** Settle ONE membership rule for a paused or blocked bundle and make `skills/resume/SKILL.md` follow it: on resume, take the bundle's members from the `BUNDLE=` line of the cursor's existing `<run-dir>/<cursor>/handoff.md` — the membership the open start records were written against — re-running only the mechanical refusals (a member now checked, gone from the plan, or already routed `hand-off-feature` drops out, the cursor itself never), and re-form the group with `bundle-cap` + `group` ONLY when no handoff exists for the cursor; this replaces T7's unconditional re-grouping, which could shrink or grow a paused bundle so the sweep closed members the run had started as `interrupted`. State plainly that a single-task handoff (no `BUNDLE=` line) means a one-member bundle, so a cap of 1 behaves exactly as before; also state that the adopt path's anchored-trailer check escalating on a legacy commit whose trailers are inline rather than on their own lines is intended, not a regression. Prose only, no sweep case, checkable by reading resume's §4 membership paragraph against `skills/run-loop/SKILL.md`'s own recovery claim (T10) for one identical rule.
  - deps: T7
  - covers: Every task in a bundle keeps its own record · A bundle is one unit of work for dispatch, review and routing
  - arch: —
  - files: skills/resume/SKILL.md
- [x] **T10** [P] **P0** Close three gaps in `skills/run-loop/SKILL.md`'s bundle prose: (a) make §3.2's statement of how a compacted or resumed session recovers `$MEMBERS` state the same rule T9 gives resume — the handoff's `BUNDLE=` line when a handoff exists, re-grouping only when none does — with no second phrasing; (b) handle `gspec-backlog.sh group` exiting non-zero at §3.2 by falling back to a group of exactly the cursor and saying so in the packet's report, never halting the loop and never guessing members; (c) anchor §4's stop-report trailer lookup to a whole-line trailer (the same `^[[:space:]]*\[orch packet:<id>\][[:space:]]*$` shape §1's drift scan uses) rather than an unanchored `--grep`, so prose that merely mentions a trailer cannot be read as a landed member. A cap of 1 must leave every touched step behaving exactly as today. Prose only, no sweep case, checkable by reading §3.2 and §4 against resume (T9) and §1.
  - deps: T6
