...
- [x] **P0**: The gspec task checkbox is the completion record, and the loop maintains it
...
- [x] **P1**: A drifted completion record is reported at preflight
...
- [x] **P0**: `backlog.done` is deleted, with no counter, tail, or replacement
...
- [x] **P0**: The tally's ✅ bucket is this session, everywhere
...
- [x] **P1**: `reconstruct` keeps `DONE=`, and nothing is populated from it
...
- [x] **P0**: A finding is scoped to packets, and there is no run-wide finding
...
- [x] **P0**: A finding is dropped at the packet boundary, and both halves go or neither
...
- [x] **P0**: A resolved finding is captured before it is dropped, by the session that resolved it
...
- [x] **P1**: Finding bodies are opt-in
...
- [x] **P0**: Neither change bumps the run-state schema, and there is no flag day
...
- [x] **P0**: Migration is detection plus interactive triage, and `apply` never deletes a finding
...
- [x] **P0**: Every changed behaviour has a case in its regression sweep
