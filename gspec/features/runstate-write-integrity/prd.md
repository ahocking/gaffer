...
- [x] **P0**: `set` cannot corrupt run-state, whatever the value contains
...
- [x] **P0**: `write` refuses structurally invalid input and leaves the existing file byte-untouched
  ...
  - **before replacing an existing run-state, `write` copies it to `.agents/run-state-prev.yaml`** — the last known good. This is the criterion that actually covers the incident: the structural checks above catch a transform that dies *mid-line*, but one that dies *between* lines yields a structurally perfect document, and `schema: 3` + `status: running` is exactly the 26 bytes the live truncation left. Detection cannot close that without correctly classifying every valid and invalid document forever; a copy needs no classification. **Shrinkage detection was designed and rejected**: measured on this repo, the `/gaffer:migrate` findings triage shrinks run-state by 61% because the findings index is 87% of the file, so any threshold that catches a stub also refuses a legitimate, shipped workflow — wrong in both directions
  - the copy is **gitignored in both `.gitignore` files**, and that is load-bearing rather than tidiness: an untracked file is swept by the pause path's `git stash --include-untracked` and read by `reconcile` as scratch on the green checkpoint, then discarded — so an unignored backup is destroyed by the very recovery path it exists to serve. Same-machine, same as run-state itself
...
- [x] **P0**: The integrity checks run on every host `runstate.sh` runs on, in POSIX shell alone
...
- [x] **P0**: Every hazard has a regression case, asserted rather than grepped
...
- [x] **P1**: The sweep's parse assertion never silently no-ops
...
- [x] **P1**: The sweep is deterministic, so a green run means the same thing every time
