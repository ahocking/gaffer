...
`${CLAUDE_PLUGIN_ROOT}/templates/report-conventions.md`.** This skill has **no shape of
its own**, so those conventions *are* its format; naming the path is not reading it. You
do **not** need `report-templates.md`: it holds the guided loop's shapes, which this never
...
- **packet boundaries** derived from the `[orch packet:<id>]` commit trailers AND
  `runstate.sh record-start`/`record-outcome`/`sweep-open` attestations — a packet the
  loop started appears even if it never committed (failed, rolled-back) or was
  interrupted mid-run, and a pause commit's trailer never implies green — all
  **bounded to that session's run window** so a prior run's committed packets are not
  folded in, and
- **token/cost** from the Claude Code transcripts (**version-fragile**,
...
**Outcome coverage is a label, never a green count.** The
...
**Main-session context.** `totals.driver_mode_context` reports the largest context of
one main-thread turn — its input, cache-write and cache-read tokens, output excluded,
...
invented threshold. This run-KICKOFF baseline is expected to disagree with
`run-digest`, which reads the MOST RECENT `enter` across every session. A later
re-entry is ASSUMED, not verified, to carry the same repo/operator setting as the
kickoff record it is compared against. `totals.driver_mode_context_diagnostics`
...
recommended value for it — whether and what to set is the operator's call.

This takes **no header tally and no glyph gutter**, since there is nothing to count
...
run), say so in words, since a `0` reads to the human as "clean".
...
Reports `metrics: on|off` (env `ORCH_METRICS`, then `metrics.enabled` in
`.agents/project-overrides.yaml`, then `metrics.sh`'s own fallback), whether `jq` and a transcript dir are present, and
...
- **Cache efficiency** — `totals.cache_hit_ratio` and the cacheCreation-vs-cacheRead
  ...
  re-loading context instead of reusing it.
- **Where spend concentrates** — the `by_agent_role` token split and the per-packet
  `tool_calls` table: which role or packet dominates.
- **Same-file overlap, via `totals.same_file_overlaps`** — a run-level count of (main session, subagent) pairs that edited the SAME
  ...
  span, and a pair counts once no matter how many files it shares. It tells the
  operator whether concurrent editing guidance is holding. `null` means
  **unmeasured** (no events in the run, or at least one Edit/Write/MultiEdit/NotebookEdit event with no `file_hash`), never a
...
- **The relay-vs-inline crossover** — whether this run's shape supports or contradicts
  ADR 0012's 40-packet crossover. A run below the
...
- **Standing-context size, via `by_agent_role.<role>.cc_shape`** — prefer this over
  cacheCreation totals, which are too noisy to steer by. `median` is the cost of one more turn;
...
- **`totals.context_invalidations`** — effort or model changed mid-context, which
  re-caches the whole prefix. If any appear, say so and note that the fix is behavioural:
...
- **`packets[].outcome` and `packets[].edits`** — both are `null` on runs that predate
  the attestation, and `null` means **unmeasured, not clean**. Read
...
- **`packets[].dispatches[]` and `totals.dispatch_waste`** — one row per implementer dispatch of a packet, each with `kind` (`initial`,
...
- **`self_host`** — a boolean indicating whether this run measured the plugin's own
  ...
  runs must NOT be averaged together, since the populations are incomparable. Absent
...
them the two options, what follows from each, your lean, and the default. A bullet
that hides a cost reads as free.
...
the machine — main sessions and subagents, every project — over a window: `spend.sh`'s
default length (its usage text states it; the report's `window` shows the one used), or
`--days N`; `--since`/`--until` (ISO, e.g. `2026-09-08T00:00:00Z`) override it. Tokens are priced from `scripts/spend-prices.json` (or `--price-table
...
Enable/disable and retention live under `metrics:` in `.agents/project-overrides.yaml`:
`enabled` (the zero-token Tier-1 event+timing spine), `otel` (Tier 0 (OTEL), optional and
deferred — not required for Tier 1) and `retain_runs` (the cap on retained
`.agents/metrics/<run-id>/` dirs). The env var `ORCH_METRICS` overrides `enabled` for a
session; `metrics.sh status` prints the result. `.agents/metrics/` is
